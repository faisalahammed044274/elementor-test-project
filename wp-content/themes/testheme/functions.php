...
function testheme_setup()
{
     ...
     * If you're building a theme based on testheme, use a find and replace
     * to change 'testheme' to the name of your theme in all the template files.
     */
    ...
    add_theme_support(
        'custom-background',
        apply_filters(
            'testheme_custom_background_args',
            array(
 ...
 */
function testheme_content_width()
{
    $GLOBALS['content_width'] = apply_filters('testheme_content_width', 640);
}
add_action('after_setup_theme', 'testheme_content_width', 0);
...
function testheme_scripts()
{
    ...
    wp_enqueue_style('testheme-style', get_stylesheet_uri(), array(), _S_VERSION);
    wp_style_add_data('testheme-style', 'rtl', 'replace');
    ...
    wp_enqueue_script('testheme-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true);
